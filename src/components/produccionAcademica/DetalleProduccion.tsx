...
export default function DetalleProduccion({
  ...
  const [downloading, setDownloading] = useState<
    "archivo" | "certificado" | null
  >(null);
  const [uploading, setUploading] = useState<"archivo" | "certificado" | null>(
    null,
  );
  ...
  const handleUpload = async (field: "archivo" | "certificado", file: File) => {
    ...
    if (file.size > MAX_UPLOAD_SIZE_BYTES) {
      setFileError(
        `El archivo supera los ${MAX_UPLOAD_SIZE_MB} MB permitidos.`,
      );
      ...
      try {
        ...
        setFileError(
          "No se pudo subir el archivo. Verifica el formato (pdf, jpg, png, docx, xlsx).",
        );
      } finally {
        setUploading(null);
      }
        ...
        <Box p={2.5} sx={{ overflowY: "auto", flexGrow: 1 }}>
          ...
          <Stack spacing={2}>
            ...
            <Campo
              ...
              value={item.linea_investigacion_nombre}
            />
            <Campo label="Revista / Evento" value={item.revista_evento} />
            <Campo
              label="Fecha de publicación"
              value={fechaFormat(item.fecha_publicacion)}
            />
