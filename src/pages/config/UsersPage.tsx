...
import {
  ...
  PersonOutlined, RefreshOutlined,
...
export default function UsersPage() {
  ...
  const openEdit   = (u: UserAdmin) => {
    setEditing(u);
    setForm({ username: u.username, email: u.email, password: '', first_name: u.first_name, last_name: u.last_name, cedula: u.cedula, telefono: u.telefono ?? '', rol: u.rol, is_active: u.is_active });
  ...
  const handleDelete = async () => {
    ...
    try { await usersService.remove(deleteId); setDeleteId(null); load(); }
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <PersonOutlined sx={{ color: theme.palette.primary.main, fontSize: 28 }} />
          <Box>
            <Typography variant="h5" sx={{ fontFamily: '"DM Sans", sans-serif', fontWeight: 700, color: theme.palette.text.primary }}>Usuarios</Typography>
            <Typography variant="caption" sx={{ color: theme.palette.text.disabled }}>Gestión de usuarios del sistema</Typography>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="Recargar"><IconButton onClick={load} size="small"><RefreshOutlined /></IconButton></Tooltip>
          <Button variant="contained" startIcon={<AddOutlined />} onClick={openCreate} sx={{ borderRadius: 2, textTransform: 'none' }}>Nuevo usuario</Button>
        </Box>
      </Box>
      ...
      {/* Search */}
      <TextField
        size="small" placeholder="Buscar por nombre, email o cédula…" value={search}
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)' }}>
                {['Nombre', 'Email', 'Cédula', 'Rol', 'Estado', 'Acciones'].map(h => (
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow key={u.id} hover>
                  ...
                  <TableCell>
                    <Chip label={ROLES.find(r => r.value === u.rol)?.label ?? u.rol} size="small"
                      sx={{ height: 22, fontSize: '0.7rem', fontWeight: 600, color: rolColor(u.rol), bgcolor: `${rolColor(u.rol)}22` }} />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    <Tooltip title="Eliminar"><IconButton size="small" color="error" onClick={() => setDeleteId(u.id)}><DeleteOutlined fontSize="small" /></IconButton></Tooltip>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        ...
        <DialogContent dividers>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            <TextField label="Nombre" value={form.first_name} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))} size="small" />
            <TextField label="Apellidos" value={form.last_name} onChange={e => setForm(f => ({ ...f, last_name: e.target.value }))} size="small" />
            <TextField label="Username" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} size="small" />
            <TextField label="Cédula" value={form.cedula} onChange={e => setForm(f => ({ ...f, cedula: e.target.value }))} size="small" />
            <TextField label="Email (@ufps.edu.co)" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} size="small" sx={{ gridColumn: '1 / -1' }} />
            {!editing && (
              <TextField label="Contraseña" type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} size="small" sx={{ gridColumn: '1 / -1' }} />
            )}
            <TextField label="Teléfono" value={form.telefono} onChange={e => setForm(f => ({ ...f, telefono: e.target.value }))} size="small" />
